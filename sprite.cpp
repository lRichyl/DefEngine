...
void render_sprite(Renderer *renderer, Sprite *spr, V2 position){
	...
	if(spr->clipping_box.x == 0 && spr->clipping_box.y == 0 && spr->clipping_box.w == 0 && spr->clipping_box.h == 0){
		render_quad(renderer, &bounding_box, &spr->info.texture, spr->info.layer, NULL,  spr->info.is_x_mirrored, spr->info.alpha, spr->info.tint, spr->info.is_y_mirrored);
		
	}else{
		render_quad(renderer, &bounding_box, &spr->info.texture, spr->info.layer, &spr->clipping_box, spr->info.is_x_mirrored, spr->info.alpha, spr->info.tint, spr->info.is_y_mirrored);
		
	}
}

void render_sprite_as_background(Renderer *renderer, Sprite *spr){
	render_quad(renderer, NULL, &spr->info.texture, spr->info.layer, NULL, spr->info.is_x_mirrored, spr->info.alpha, spr->info.tint, spr->info.is_y_mirrored);
}
...
void render_animation(Renderer *renderer, AnimatedSprite *anim_spr, V2 position){
	...
	render_quad(renderer, &bounding_box, &anim_spr->info.texture, anim_spr->info.layer, &anim_spr->clipping_boxes[anim_spr->current_frame],  anim_spr->info.is_x_mirrored, anim_spr->info.alpha, anim_spr->info.tint, anim_spr->info.is_y_mirrored);
