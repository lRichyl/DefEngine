...
namespace def {
	void render_sprite(Renderer *renderer, Sprite *spr, VEC_2D position, ShaderProgram *shader){
	...
	void render_queue_sprite(DefArray<RenderCommand> *command_list, Renderer *renderer, Sprite *spr, VEC_2D position, ShaderProgram *shader){
		RECT bounding_box = {position.x, position.y, spr->info.size.x, spr->info.size.y};
		if(spr->clipping_box.x == 0 && spr->clipping_box.y == 0 && spr->clipping_box.w == 0 && spr->clipping_box.h == 0){
			if(!shader)
